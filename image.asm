...
charcopy:
	jsr charload		// Load character
	jsr charstore		// Save character
	inx
	bne charcopy		// After 256 characters have been written, increment
	inc charload + 2	// the address by $100
	inc charstore + 2
	iny					// Keep track of how much data has been written
	cpy #$4				// and break the loop after 1000 bytes
	bne charcopy

	jmp colours			// Jump to colour writing section

colours:				// Set registers before copying colour data
	ldx #$0
	ldy #$0
...
colourcopy:
	...
	jsr colourstore
	inx
	bne colourcopy		// After 256 colours have been written, increment
	inc colourload + 2	// the address by $100
	inc colourstore + 2
	iny					// Keep track of how much data has been written
	cpy #$4				// and break the loop after 1000 bytes
	bne colourcopy
	...
	lda #$18			// Set multicolour mode
	sta $d016

	jmp main			// Jump to main program

charload:				// Subroutine to load character data
	lda $3f40,x			// into Accumulator
	rts

charstore:				// Subroutine to save character data
	sta $0400,x
	rts

colourload:				// Subroutine to load colour data
	lda $4328,x			// into Accumulator
	rts

colourstore:			// Subroutine to save colour data
	sta $d800,x
	rts
...
irq1:					// Begin double interrupt. This ensures
	...
	sta registerstore	// Store accumulator
	txa
	sta registerstore+1	// Store x register
	tya
	sta registerstore+2	// Store y register
...
waitloop:
	...
	lda registerstore	// Restore Accumulator
